...
| DB (개발) | h2 (파일 모드) | Spring Boot 내장 | 서버 재시작 후 데이터 유지, AUTO_SERVER=TRUE로 DataGrip 동시 접속 가능 |
...
> **Rate Limit**: Upbit 공개 API는 초당 최대 10회 요청 허용.
> `FeignConfig`의 Guava `RateLimiter`를 **8 req/sec** (20% 여유)로 설정하여 429 오류를 방지한다.
> `count` 파라미터 최대값은 200이며, 더 많은 데이터가 필요할 경우 내부적으로 여러 번 페이징 요청한다.
...
│       ├── service/BacktestExecutionService.java   # LinearTransactionCostModel(0.05%) 적용
...
| 필드 | 설명 |
|------|------|
| totalReturn | 총 순수익률 (수수료 차감 후) |
| maxDrawdown | 최대낙폭 (%) |
| winRate | 승률 (%) |
| sharpeRatio | 샤프지수 (포지션 수익률 평균 / 표준편차) |
| calmarRatio | 칼마지수 (총수익률 / 최대낙폭) |
| profitFactor | 수익 팩터 (순이익 / 순손실) |
| totalTrades | 총 거래 횟수 |
| buyAndHoldReturn | Buy & Hold 벤치마크 수익률 |
...
| 항목 | 선택 | 이유 |
|------|------|------|
| 기술지표 | TA4J 0.14 | 직접 구현 대신 검증된 계산, Java 생태계 표준, 50+ 지표 내장 |
| 수수료 모델 | LinearTransactionCostModel(0.0005) | 업비트 KRW 마켓 수수료 0.05% 반영, TA4J 내장 모델 활용 |
| DB | H2 파일 모드 | 서버 재시작 후에도 캔들 데이터 유지, 매번 API 재수집 불필요 |
| H2 AUTO_SERVER | AUTO_SERVER=TRUE | IntelliJ / DataGrip 동시 접속 허용 (서버 모드 자동 전환) |
...
| Rate Limit | Guava RateLimiter(8.0) | Upbit 한도 10 req/sec에서 20% 여유, 429 오류 방지 |
...
| Phase | 내용 | 상태 |
|-------|------|------|
| 1 | Upbit 캔들 API 추가 + Candle 엔터티 + DB 저장 + @Scheduled 수집 | ✅ 완료 |
| 2 | TA4J 통합 + SMA / EMA / RSI / MACD / BB 지표 계산 서비스 | ✅ 완료 |
| 3 | 전략 구현 (골든크로스, RSI 과매도, MACD 크로스, 볼린저밴드, 스캘핑) | ✅ 완료 |
